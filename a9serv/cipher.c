#include <sys/types.h>
#include "myval.h"

static const unsigned char KEY_TABLE[] = {
  
};

/* PPPP local PSK hash for DID prefix EEE / check string "SHIX". */
static const unsigned char key[] = {0x3c, 0xc4, 0x68, 0x0a};

void encode(myval *buf) {
  unsigned char *ptr = buf->mv_data;
  unsigned char index;
  unsigned char prevByte = 0;
  int i;
  for (i=0; i<buf->mv_size; i++) {
  	index = (key[prevByte & 0x03] + prevByte);
	*ptr = *ptr ^ KEY_TABLE[index];
	prevByte = *ptr++;
  }
}

void decode(myval *buf) {
  unsigned char *ptr = buf->mv_data;
  unsigned char index;
  unsigned char prevByte = 0;
  int i;
  for (i=0; i<buf->mv_size; i++) {
  	index = (key[prevByte & 0x03] + prevByte);
	prevByte = *ptr;
	*ptr = *ptr ^ KEY_TABLE[index];
	ptr++;
  }
}
