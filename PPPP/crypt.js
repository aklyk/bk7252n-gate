const KEY_TABLE = [
  
];

function keyFromPsk(psk) {
  const key = [0, 0, 0, 0]
  for (const ch of Buffer.from(psk, 'ascii')) {
    key[0] = (key[0] + ch) & 0xff
    key[1] = (key[1] - ch) & 0xff
    key[2] = (key[2] + Math.floor(ch / 3)) & 0xff
    key[3] = key[3] ^ ch
  }
  return key
}

const defaultKey = keyFromPsk('camera')

function normalizeKey(keyOrPsk) {
  if (!keyOrPsk) return defaultKey
  if (Array.isArray(keyOrPsk)) return keyOrPsk
  return keyFromPsk(keyOrPsk)
}

function encrypt(buf, keyOrPsk) {
  const key = normalizeKey(keyOrPsk)
  let newbuff = []
  let previousByte = 0
  for (const [position] of buf.entries()) {
    const index = (key[previousByte & 0x03] + previousByte) & 0xFF
    const r = buf[position] ^ KEY_TABLE[index]
    newbuff.push(r)
    previousByte = r
  }
  return Buffer.from(newbuff)
}

function decrypt(buf, keyOrPsk) {
  const key = normalizeKey(keyOrPsk)
  let newbuff = []
  let previousByte = 0
  for (const [position] of buf.entries()) {
    const index = (key[previousByte & 3] + previousByte) & 255
    previousByte = buf[position]
    const r = buf[position] ^ KEY_TABLE[index]
    newbuff.push(r)
  }

  return Buffer.from(newbuff)
}

// let b = Buffer.from("Hello World");
// console.log(b);
// let e = encrypt(b)
// console.log(e)
// console.log(decrypt(e))
// console.log(decrypt(e).toString())

function dh(m) {
  let b = Buffer.from(m, "hex")
  let p = decrypt(b)
  console.log(m)
  console.log(p.toString("hex"));
  console.log(p.toString("ascii"));
  console.log("\n");
}

function eh(m) {
  let b = Buffer.from(m, "hex")
  let p = encrypt(b)
  console.log(m)
  console.log(p.toString("hex"));
  console.log(p.toString("ascii"));
  console.log("\n");
}

module.exports = {
  encrypt,
  decrypt,
  keyFromPsk,
  dh,
  eh
}
